...
namespace AvalonStudio.Projects.OmniSharp.MSBuild
{
    ...
    public static class MSBuildEnvironment
    {
        ...
        {
            var monoMSBuildDirPath = @"C:\Program Files\dotnet\sdk\2.0.0-preview2-006497\";
            ...
            var msbuildExePath = Path.Combine(monoMSBuildDirPath, "MSBuild.dll");
            if (!System.IO.File.Exists(msbuildExePath))
            {
        ...
        private static string FindLocalMSBuildFromSolution()
        {
            ...
            while (!System.IO.File.Exists(Path.Combine(current, "OmniSharp.sln")))
            {
